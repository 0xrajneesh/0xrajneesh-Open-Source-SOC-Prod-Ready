...
Create a Jira issue for Graylog security events requiring investigation.

## Graylog Event Condition

```text
Graylog event matches investigation policy
```
...
1. Receive the event.
2. Create a deterministic fingerprint from event type, host, user, and indicator.
3. Check whether a matching open ticket already exists.
4. Create a Jira Security issue when no duplicate exists.
5. Add Graylog event details and investigation links.
6. Notify the analyst channel.
