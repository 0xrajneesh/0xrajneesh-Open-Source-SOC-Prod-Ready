...
Automatically enrich suspicious source or destination IP addresses.

## Graylog Event Condition

```text
Graylog event contains a suspicious external IP
```
...
1. Webhook
2. Extract IP
3. HTTP Request — AbuseIPDB
4. HTTP Request — VirusTotal
5. Merge
6. Edit Fields — risk summary
7. Notify / Ticket
...
1. Receive the Graylog event.
2. Extract `src_ip` or `dst_ip`.
3. Query one or more approved threat-intelligence APIs.
4. Merge the returned reputation information.
5. Create a concise risk summary.
6. Append enrichment to the SOC ticket or notification.
