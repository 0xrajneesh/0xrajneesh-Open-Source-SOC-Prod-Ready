# Isolate Endpoint with EDR

## Objective

Request endpoint isolation through an EDR API after a confirmed compromise.

## Graylog Event Condition

```text
Critical endpoint compromise event
```

## n8n Nodes

1. Webhook
2. Extract hostname/device ID
3. EDR device lookup
4. IF — valid endpoint
5. Approval
6. EDR isolate API
...
1. Receive the endpoint security event.
2. Map Graylog hostname to the EDR device identifier.
3. Confirm the endpoint is not on an exclusion list.
4. Require approval for production containment.
5. Call the EDR isolation API.
6. Poll or query isolation status and add the result to the incident.
