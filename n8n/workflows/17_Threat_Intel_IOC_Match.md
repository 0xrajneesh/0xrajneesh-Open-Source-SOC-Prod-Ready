...
Respond when Graylog logs match a known malicious IOC.

## Graylog Event Condition

```text
Event contains IP/domain/hash matching a threat-intel watchlist
```
...
4. Check confidence/age
5. IF — actionable
6. Search related events via Graylog API
7. Create Ticket
8. Notify
...
1. Trigger when Graylog matches a maintained IOC list.
2. Identify whether the indicator is an IP, domain, URL, or hash.
3. Validate confidence and intelligence age.
4. Query Graylog for related activity around the event time.
5. Create one enriched investigation ticket instead of sending a raw IOC alert.
