# SOC Alert Escalation Timer

## Objective

Escalate an incident if an analyst has not acknowledged it within the required time.

## Graylog Event Condition

```text
High-severity event requiring acknowledgement
```
...
1. Webhook
2. Create Ticket
3. Send Initial Alert
4. Wait
5. Check ticket/ack status
6. IF — acknowledged
7. Escalate to Tier 2 / Manager
...
1. Create the incident and notify the primary SOC channel.
2. Wait for your SLA acknowledgement period.
3. Query the ticketing system or acknowledgement store.
4. If acknowledged, finish the workflow.
5. If not acknowledged, escalate to the next response tier.
6. Record each escalation timestamp.
