...
Detect and investigate sudden spikes in denied network traffic.

## Graylog Event Condition

```text
Count of firewall deny events exceeds baseline/threshold
```
...
1. Webhook
2. Normalize count
3. Extract top source/destination
4. IP reputation enrichment
5. IF — suspicious pattern
6. Ticket / Notify

## Workflow

```text
Graylog Firewall Aggregation
           ↓
         Webhook
           ↓
      Deny Count + IPs
...
1. Create an aggregation event in Graylog for deny spikes.
2. Include top source/destination fields where possible.
3. Enrich external IPs.
4. Differentiate scanners, known monitoring systems, and unknown sources.
5. Open an investigation ticket when the pattern is suspicious.
