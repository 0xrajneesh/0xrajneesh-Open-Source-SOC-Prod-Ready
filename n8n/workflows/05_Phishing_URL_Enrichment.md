...
Analyze suspicious URLs found in email, proxy, or endpoint events.

## Graylog Event Condition

```text
Graylog event contains a suspicious URL
```
...
3. HTTP Request — URL reputation
4. HTTP Request — DNS/domain enrichment
5. Merge
6. IF — suspicious
7. Ticket / Notify
...
1. Create a Graylog event for suspicious URLs.
2. Send the event to n8n.
3. Extract the URL/domain and sanitize it before sending to third-party services.
4. Perform reputation checks.
5. Classify the URL as benign, suspicious, or malicious.
6. Notify analysts with supporting enrichment.
